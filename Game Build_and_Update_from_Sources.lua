...
-- Escape Lua pattern magic characters so we can safely build patterns from filenames/bases.
...
-- From e.g. "a1D-fX_ProfClary_Take001.mp3" or "00001_Narrator_Take03.wav" or "..._TakeX" -> base without take suffix
local function base_from_filename(fn)
  local s = tostring(fn or "")

  -- Strip any path
  s = s:gsub("^.+[\\/]", "")

  -- Strip extension if present (case-insensitive)
  s = s:gsub("%.[Ww][Aa][Vv]$", "")
  s = s:gsub("%.[Mm][Pp]3$", "")

  -- Strip trailing take token
  s = s:gsub("_[Tt][Aa][Kk][Ee][Xx]$", "")
  s = s:gsub("_[Tt][Aa][Kk][Ee]%-?%d+$", "")
...
local function extract_base_from_note(note)
  ...
  -- Old style: note itself looks like a base (no spaces, contains at least one underscore)
  if not note:find("%s") and note:find("_") then
    return note
  end
...
local function extract_take_number_from_name(name)
  ...
  local num = last
  local m = last:match("^[Tt]ake%-?(%d+)$")  -- e.g. "Take03" -> "03"
  if m and m ~= "" then
    num = m
  end
...
local function add_take_marker_for_row(take, base, file_name_stem)
  ...
  reaper.SetTakeMarker(take, -1, dialogue, 0.0)  -- position 0 = start of take
end

-- Given a base (e.g. "a1D-fX_ProfClary" or "00001_Narrator"), find all available take files in ./Sources.
-- Accept Take<digits> with ANY zero-padding (e.g. Take1, Take01, Take001, Take0001...). Prefer WAV over MP3 for the same take index.
local function collect_takes_for_base(base)
  local files = {}
  if not base or base == "" then return files end

  local esc_base = escape_lua_pattern(base)

  -- take_index -> { wav=path, mp3=path }
  local best = {}
  ...
  while true do
    ...
    local lower = name:lower()
    local is_wav = (lower:sub(-4) == ".wav")
    local is_mp3 = (lower:sub(-4) == ".mp3")

    if is_wav or is_mp3 then
      local stem = name:sub(1, -5) -- strip extension
      local digits = stem:match("^" .. esc_base .. "_[Tt][Aa][Kk][Ee]%-?(%d+)$")
      if digits then
        local idx = tonumber(digits)
        if idx then
          local entry = best[idx] or { wav=nil, mp3=nil }
          local full_path = join(sources_dir, name)
          if is_wav then
            entry.wav = entry.wav or full_path
          else
            entry.mp3 = entry.mp3 or full_path
          end
          best[idx] = entry
        end
      end
  ...
  for _, k in ipairs(idxs) do
    local entry = best[k]
    local chosen = entry and (entry.wav or entry.mp3)
    if chosen then
      files[#files + 1] = chosen
    end
  end
...
-- Determine which track a CSV row should go on.
-- Priority:
--   1) row.track_override (CSV col 4)
--   2) character token inferred from base (everything between first and second underscore)
--   3) speaker with spaces removed
local function infer_track_name(row, base)
  if row and row.track_override and row.track_override ~= "" then
    return row.track_override
  end

  if base and base ~= "" then
    local token = base:match("^[^_]+_([^_]+)")
    if token and token ~= "" then
      return token
    end
  end

  local sp = row and row.speaker or ""
  sp = tostring(sp):gsub("%s+", "")
  if sp == "" then sp = "Unknown" end
  return sp
end

-- Create an empty placeholder item at pos with given length; 'note_text' is the full item note.
-- Also creates a single EMPTY take named "<base>_TakeX" so Route B can recover the base even if SWS notes are unavailable.
local function create_placeholder_item(track, pos, length, note_text, base_for_fallback)
  ...
  -- Create a single empty take so the item isn't "take-less".
  local take = reaper.AddTakeToMediaItem(item)
  if take and base_for_fallback and base_for_fallback ~= "" then
    reaper.GetSetMediaItemTakeInfo_String(take, "P_NAME", tostring(base_for_fallback) .. "_TakeX", true)
  end

  -- Ensure looping is off on placeholders too.
...
local function route_A(rows)
  ...
  for _, row in ipairs(rows) do
    ...
    local track_name = infer_track_name(row, base)
    ...
    if #files > 0 then
      ...
      local note_text = build_item_note_for_row(base, row)
      create_placeholder_item(track, cursor, seconds, note_text, base)
  ...
  for ti = 0, tr_cnt - 1 do
    ...
    for ii = it_cnt - 1, 0, -1 do
      ...
      if is_placeholder then
        ...
        -- Build a set of take indices already present on this item.
        local present = {}
        local esc_base = escape_lua_pattern(base)

        for tki = 0, takes - 1 do
          local tk = reaper.GetMediaItemTake(item, tki)
          if tk then
            local _, tname = reaper.GetSetMediaItemTakeInfo_String(tk, "P_NAME", "", false)
            tname = tostring(tname or "")
            local digits = tname:match("^" .. esc_base .. "_[Tt][Aa][Kk][Ee]%-?(%d+)$") or tname:match("_[Tt][Aa][Kk][Ee]%-?(%d+)$")
            if digits then
              local idx = tonumber(digits)
        ...
        -- Enumerate ALL take files for this base and add any that aren't already present.
        local all_files = collect_takes_for_base(base)

        for _, chosen in ipairs(all_files) do
          local file_stem = chosen:match("([^" .. SEP .. "]+)$") or chosen
          file_stem = file_stem:gsub("%.[Ww][Aa][Vv]$", "")
          file_stem = file_stem:gsub("%.[Mm][Pp]3$", "")

          local digits = file_stem:match("^" .. esc_base .. "_[Tt][Aa][Kk][Ee]%-?(%d+)$")
          local idx = digits and tonumber(digits) or nil

          if idx and not present[idx] then
