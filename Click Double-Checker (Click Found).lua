-- @description Tails Click Check (Mark Click): drop yellow "Click" marker at previous tails marker, advance tails marker, play last 1s (+0.5s)
-- @author David Winter
-- @version 1.1
...
local CLICK_MARKER_NAME = "Click"
local CLICK_COLOR = reaper.ColorToNative(255, 255, 0) | 0x1000000  -- yellow
...
local function add_click_marker_at_time(t)
  reaper.AddProjectMarker2(PROJ, false, t, 0, CLICK_MARKER_NAME, -1, CLICK_COLOR)
end

local function replace_tails_marker_and_mark_click(existingMarker, newPos)
  if existingMarker and existingMarker.idx then
    local oldPos = existingMarker.pos
    reaper.DeleteProjectMarker(PROJ, existingMarker.idx, false)
    add_click_marker_at_time(oldPos)
  end
  reaper.AddProjectMarker2(PROJ, false, newPos, 0, MARKER_NAME, -1, 0)
end
...
local function main()
  ...
  if existingMarker then
    ...
    if not startPos then
      reaper.PreventUIRefresh(-1)
      reaper.Undo_EndBlock("Tails Click Check (Mark Click)", -1)
  ...
  set_repeat_off()
  replace_tails_marker_and_mark_click(existingMarker, X)

  reaper.UpdateArrange()
  reaper.PreventUIRefresh(-1)
  reaper.Undo_EndBlock("Tails Click Check (Mark Click): drop click marker + advance tails marker + play", -1)
